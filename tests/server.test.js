...
async function main() {
  ...
  if (await test('GET /health identifies the app and version', async () => {
    ...
    assert.deepStrictEqual(jsonBody(res), { ok: true, app: 'eddie', version: '9.9.9-test' });
  })) passed++; else failed++;
  ...
  fs.rmSync(path.join(os.tmpdir(), 'eddie-outside.txt'), { force: true });
