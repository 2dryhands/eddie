...
// Returns null when there's no usable byte range (caller streams the whole
// file with 200), { unsatisfiable: true } for a 416, or { start, end }
// (inclusive) for a 206. The two forms must NOT be conflated:
//   `bytes=N-`  -> from offset N to the end
//   `bytes=-N`  -> the LAST N bytes (suffix). N is a length, not an offset.
// Safari asks for the trailing MP4 `moov` atom with the suffix form, so
// mishandling it (a stray 416 or the wrong window) wedges playback — which is
// exactly the Safari media breakage this Range support exists to fix.
function parseByteRange(rangeHeader, size) {
  ...
  const hasStart = m[1] !== '';
  const hasEnd = m[2] !== '';
  if (!hasStart && !hasEnd) return null; // "bytes=-" is malformed → serve whole
  let start;
  let end;
  if (!hasStart) {
...
// frame-ancestors 'self' blocks clickjacking (embedding an Eddie page inside
// a third-party site's iframe) while still allowing the one legitimate case:
// the canvas chrome framing the reviewed artifact from the same origin.
// Applied to every HTML response, csp:true or not, so the reviewed-artifact
// pages (which need csp:false to keep their own inline scripts working)
// still ship this one header.
function sendHtml(res, statusCode, html, { csp = true } = {}) {
  const headers = { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' };
  headers['content-security-policy'] = csp
    ? "default-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; frame-src 'self'; frame-ancestors 'self'"
...
function createPlanCanvasServer({
  ...
  const selfWrites = new Map(); // key -> ts of our own last /save write (suppress the watcher's reload for it)
  let idleTimer = null;
  let closed = false;
  ...
  function armIdleTimer() {
    ...
    idleTimer = setTimeout(() => {
      if (connectionCount() === 0 && !closed) {
        log('[eddie] idle timeout reached, shutting down');
  ...
  function watchSession(session) {
    ...
    try {
      const watcher = fs.watch(dir, (eventType, filename) => {
        ...
        // Skip our own live-edit save: reloading the iframe here would blow
        // away the reviewer's cursor and in-flight typing mid-edit.
        if (Date.now() - (selfWrites.get(session.key) || 0) < 1200) return;
  ...
  async function handleApi(req, res, url) {
    ...
    if (req.method === 'GET' && pathname === '/api/await') {
      ...
      if (first.status !== 'waiting') {
        if (first.status === 'feedback') {
          workingKeys.add(key);
          broadcast(key, 'task-sync', { tasks: store.get(key).tasks || [] });
        }
        broadcastPresence(key);
    ...
    const sessionMatch = pathname.match(/^\/api\/session\/([a-f0-9]{12})\/(feedback|end|reply|resolve|save)$/);
    if (sessionMatch && req.method === 'POST') {
      ...
      if (action === 'resolve') {
        const body = await readJsonBody(req);
        const result = store.resolveTasks(key, body.resolutions);
        if (!result) return sendJson(res, 404, { error: 'unknown session' });
        broadcast(key, 'task-sync', { tasks: store.get(key).tasks || [] });
        return sendJson(res, 200, { status: 'resolved', updated: result.updated });
      }

      if (action === 'save') {
        // Live-edit write-back. Only HTML artifacts round-trip: a markdown
        // artifact is served as rendered HTML that can't be turned back into
        // its source, so its edits stay agent-mediated. Bigger cap than the
        // default — a full standalone HTML page can exceed 1 MB.
        const body = await readJsonBody(req, 16 * 1024 * 1024);
        if (typeof body.html !== 'string' || !body.html) {
          return sendJson(res, 400, { error: 'html is required' });
        }
        const ext = path.extname(session.file).toLowerCase();
        if (ext !== '.html' && ext !== '.htm') {
          return sendJson(res, 409, { error: 'artifact is not directly editable (only .html/.htm write back to disk)' });
        }
        try {
          selfWrites.set(key, Date.now()); // suppress the watcher reload for this write
          fs.writeFileSync(session.file, body.html);
        } catch (err) {
          return sendJson(res, 500, { error: 'save failed: ' + err.message });
        }
        return sendJson(res, 200, { status: 'saved', bytes: Buffer.byteLength(body.html) });
      }
    }
  ...
  function handleEvents(req, res, key) {
    ...
    res.write(`event: task-sync\ndata: ${JSON.stringify({ tasks: session.tasks || [] })}\n\n`);
  ...
  function serveArtifact(req, res, key, assetPath) {
    const session = store.get(key);
    ...
    // Range support so <video>/<audio> can stream and seek (Safari требует 206,
    // иначе media вообще не проигрывается). Без Range-заголовка отдаём файл целиком.
    const parsed = parseByteRange(req && req.headers && req.headers.range, stat.size);
    ...
    Promise.resolve()
      .then(() => {
        if (req.method === 'GET' && pathname === '/health') {
          return sendJson(res, 200, { ok: true, app: 'eddie', version });
        }
        ...
        if (req.method === 'GET' && artifactMatch) {
          ...
          return serveArtifact(req, res, artifactMatch[1], assetPath || null);
        }
