...
function canvasCss() {
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .changes-act{border:1px solid var(--orange);background:var(--orange-glow);color:var(--orange)}
  .changes-act:hover{background:var(--orange);color:#fff}
...
(() => {
  ...
  const actionBtns = [$('send'), $('changes'), $('approve')];
  ...
  // Verdicts (approve / request-changes) are chrome-button-only by design:
  // the artifact iframe is sandboxed but still runs script the artifact
  // author (or anything injected into it) controls, so it must never be
  // able to smuggle a verdict into the queue via postMessage — only the
  // chrome's own Send/Request changes/Approve buttons may add one. Anything
  // the iframe queues is silently downgraded to "ignored" if it isn't one
  // of the three legitimate human-authored kinds.
  ...
  function renderTasks() {
    ...
    $('tasksHeader').style.display = active.length ? '' : 'none';
  ...
  // otherwise the empty-state onboarding lines (and localized verdict
  // labels) would stay stuck in whichever language was active when they
  // were first drawn.
  ...
  function renderChat(entries) {
    ...
    if (!entries.length) {
      ...
      [t('onboarding1'), t('onboarding2'), t('onboarding3')].forEach(line => {
  ...
  $('changes').addEventListener('click', () => send('request-changes'));
  ...
  function connectEvents() {
    ...
    es.addEventListener('chat-sync', e => { chatEntries = JSON.parse(e.data).chat || []; renderChat(chatEntries); });
    es.addEventListener('task-sync', e => { tasks = JSON.parse(e.data).tasks || []; renderTasks(); });
...
<div class="layout">
  ...
  <aside class="panel">
    <h2 id="tasksHeader" data-i18n="tasksHeader" style="display:none"></h2>
    ...
    <div class="composer">
      ...
      <div class="actions">
        ...
        <button id="changes" class="act changes-act" type="button" data-i18n-title="requestChangesHint">🔧 <span data-i18n="requestChanges">Request changes</span><span class="n"></span></button>
        <button id="approve" class="act approve-act" type="button" data-i18n-title="approveHint">✅ <span data-i18n="approve">Approve</span><span class="n"></span></button>
      </div>
