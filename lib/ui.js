...
(() => {
  ...
  function armReadyWatch(delay) {
    ...
    readyTimer = setTimeout(() => {
      ...
      if (readyRetries >= READY_MAX_RETRIES) {
        console.warn('[eddie] artifact never signalled ready after ' + READY_MAX_RETRIES + ' reloads — hit Reload to retry');
        return;
      }
      readyRetries++;
      reloadArtifact();
  ...
  frame.addEventListener('load', () => { if (!artifactReady) armReadyWatch(2000); });
