...
(() => {
  ...
  function renderQueue() {
    ...
    queue.forEach((item, i) => {
      ...
      txt.className = 'txt';
      txt.textContent = item.kind === 'verdict' ? (item.verdict === 'approve' ? t('approve') : t('requestChanges')) + (item.text ? ': ' + item.text : '') : item.text;
  ...
  function renderTasks() {
    ...
    const draw = (host, list) => {
      ...
      for (const task of list) {
        ...
        txt.textContent = (task.anchor && task.anchor.snippet ? '[' + task.anchor.snippet.slice(0, 40) + '] ' : '') + task.text;
...
<header class="bar">
  ...
  <div id="presence" class="presence" data-state="waiting"><span class="dot"></span><span class="label" data-i18n="presenceWaiting">agent not connected</span></div>
...
${sessions.length ? `<table><thead><tr><th>Artifact</th><th>Path</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table>` : '<p class="empty">No sessions yet. Ask your agent to open a plan with eddie.</p>'}
</body>
</html>`;
}
