...
function canvasCss() {
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .blank-reload{margin-top:16px;height:34px;padding:0 18px;border-radius:6px;border:1px solid var(--accent);background:var(--accent);color:#fff;font-size:12.5px;font-weight:600;cursor:pointer;transition:all .12s}
  .blank-reload:hover{background:var(--accent-dim);border-color:var(--accent-dim)}
  .save-status{font-size:11px;color:var(--green);white-space:nowrap;opacity:.9}
...
(() => {
  ...
  window.addEventListener('message', e => {
    ...
    if (msg.type === 'pc:queue' && isQueueableItem(msg.item)) addToQueue(msg.item);
    ...
    else if (msg.type === 'pc:save' && typeof msg.html === 'string') scheduleSave(msg.html);
    else if (msg.type === 'pc:clean' && typeof msg.html === 'string') downloadClean(msg.html);
      ...
      postToFrame({ type: 'pc:set-editable', editable: boot.editable });
      applyLang();
  ...
  // --- live edit: save-to-disk + clean download (HTML artifacts only) ------
  // The iframe posts pc:save with a cleaned copy of the document on every
  // edit; we debounce it into a single write to the source file. The server
  // suppresses its own watcher reload for that write so the cursor survives.
  // Download asks the iframe for a fresh clean copy and hands it to the top
  // window (the sandboxed iframe can't trigger a download itself).
  const saveStatusEl = $('saveStatus');
  const downloadBtn = $('downloadBtn');
  if (boot.editable) downloadBtn.style.display = '';
  let saveDebounce = null;
  let pendingHtml = null;
  function showSaveStatus(ok) {
    saveStatusEl.style.display = '';
    saveStatusEl.classList.toggle('fail', !ok);
    saveStatusEl.textContent = t(ok ? 'savedToFile' : 'saveToFileFailed');
  }
  function scheduleSave(html) {
    if (!boot.editable) return;
    pendingHtml = html;
    clearTimeout(saveDebounce);
    saveDebounce = setTimeout(flushSave, 350);
  }
  async function flushSave() {
    ...
    pendingHtml = null;
    try {
      const res = await fetch('/api/session/' + key + '/save', {
        method: 'POST',
  ...
  function downloadClean(html) {
    const blob = new Blob([html], { type: 'text/html;charset=utf-8' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = (boot.file || 'artifact.html').split('/').pop() || 'artifact.html';
    document.body.appendChild(a);
    a.click();
    setTimeout(() => { URL.revokeObjectURL(a.href); a.remove(); }, 0);
  }
  downloadBtn.addEventListener('click', () => postToFrame({ type: 'pc:collect-clean' }));
  ...
  // --- session controls ------------------------------------------------
  $('reloadBtn').addEventListener('click', () => { readyRetries = 0; hideBlankOverlay(); reloadArtifact(); });
  ...
  // --- artifact readiness watchdog -------------------------------------
  // A sandboxed artifact iframe can paint blank on first load: the initial
  // navigation races the directory watcher's reload broadcast (agent writes
  // the file, opens the canvas, then tweaks it) or a transient hiccup drops
  // the load, leaving an empty white frame until the human hits Reload. The
  // artifact SDK posts 'pc:ready' on every successful load, so a missing
  // 'pc:ready' shortly after a load means "blank" — self-heal by reloading a
  // bounded number of times instead of stranding the reviewer on white.
  let artifactReady = false;
  let readyRetries = 0;
  let readyTimer = null;
  const READY_MAX_RETRIES = 2;
  const blankOverlay = $('blankOverlay');
  function showBlankOverlay() { if (blankOverlay) blankOverlay.classList.add('show'); }
  function hideBlankOverlay() { if (blankOverlay) blankOverlay.classList.remove('show'); }
  function clearReadyWatch() { if (readyTimer) { clearTimeout(readyTimer); readyTimer = null; } }
  function armReadyWatch(delay) {
    clearReadyWatch();
    if (artifactReady || ended) return;
    readyTimer = setTimeout(() => {
      readyTimer = null;
      if (artifactReady || ended) return;
      if (readyRetries >= READY_MAX_RETRIES) {
        // Auto-heal exhausted (Safari can wedge a sandboxed iframe blank).
        // Surface a visible, one-click recovery instead of a silent white frame.
        console.warn('[eddie] artifact never signalled ready after ' + READY_MAX_RETRIES + ' reloads — showing manual recovery');
        showBlankOverlay();
        return;
      }
      readyRetries++;
  ...
  function onFrameLoad() { if (!artifactReady) armReadyWatch(2000); }
  // Recreate the iframe element rather than reassigning .src: Safari often
  // leaves a sandboxed iframe blank after a mid-flight src swap but paints a
  // fresh element cleanly, and a new node drops any wedged navigation state.
  function reloadArtifact() {
    ...
    fresh.setAttribute('data-artifact-src', base);
    fresh.src = base + '?t=' + Date.now();
    fresh.addEventListener('load', onFrameLoad);
    frame.replaceWith(fresh);
    frame = fresh;
    armReadyWatch(2500);
  }
  // Coalesce bursts of reload triggers (the agent often writes the file a few
  // times right after opening) into one reload, so rapid src churn can't
  // itself wedge the frame.
  let reloadDebounce = null;
  function requestReload() {
    clearTimeout(reloadDebounce);
    reloadDebounce = setTimeout(reloadArtifact, 140);
  }
  const blankReloadBtn = $('blankReload');
  if (blankReloadBtn) blankReloadBtn.addEventListener('click', () => {
    hideBlankOverlay();
    readyRetries = 0;
    reloadArtifact();
  });
  // Re-arm on every completed navigation; a genuine blank still fires 'load'
  // but never 'pc:ready'. A healthy load clears this within a few ms.
  ...
  applyLang();
  connectEvents();
  // Cover the case where the initial navigation blanked before any 'load'
  // fired (or fired before this listener attached) — longer grace, then heal.
  armReadyWatch(3500);
})();`;
}
...
function renderCanvasHtml(session, { clientPath = '/client.js', cssPath = '/canvas.css' } = {}) {
  ...
  const bootstrap = JSON.stringify({
    ...
    chat: session.chat,
    tasks: session.tasks || [],
    editable: /\.html?$/i.test(session.file),
    dicts: DICTS
  }).replace(/</g, '\\u003c');
...
<header class="bar">
  ...
  <button id="downloadBtn" class="icon-btn" type="button" data-i18n="download" data-i18n-title="downloadHint" style="display:none">Download</button>
...
<div class="layout">
  <main class="frame">
    ...
    <div id="blankOverlay" class="overlay"><div class="card"><h3 data-i18n="blankTitle">Preview didn't load</h3><p data-i18n="blankBody"></p><button id="blankReload" class="blank-reload" type="button" data-i18n="blankReload">Reload preview</button></div></div>
  </main>
  <aside class="panel">
